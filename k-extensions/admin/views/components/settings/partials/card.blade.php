...
{{--
    ...
    daisyUI's checkbox-based `collapse` component: the enable/disable button lives inside the
    same title row that expands/collapses the card, and needs its own `@click.stop` so toggling
    enablement doesn't also flip the collapse -- a checkbox covering the whole title wouldn't
    let a nested button opt out of it. Rendered both by settings/index.blade.php (once per
        ...
        <div class="flex items-center gap-3 cursor-pointer" @click="open = ! open">
            ...
            @unless ($extension['cannotBeDisabled'])
                <button type="button"
                        ...
                        hx-swap="outerHTML"
                        class="btn btn-sm {{ $extension['enabled'] ? 'btn-ghost' : 'btn-primary' }}">
                    {{ $extension['enabled'] ? __('kopling-admin::messages.disable') : __('kopling-admin::messages.enable') }}
                </button>
            @endunless
        </div>
