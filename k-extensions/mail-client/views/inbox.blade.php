...
@section('content')
    ...
    <div class="drawer lg:drawer-open h-[calc(100vh-8rem)]">
        <input id="mail-drawer" type="checkbox" class="drawer-toggle" {{ $selected === null ? 'checked' : '' }} />

        {{--
            `htmx:after:settle` fires on the swapped-in element itself, not on whatever triggered
            the request (CLAUDE.md gotcha) -- this div is the actual hx-target for every thread
            card's hx-get (messages/list.blade.php) and survives across swaps unchanged (only its
        ...
        <div class="drawer-content h-full overflow-y-auto" id="mail-reading-pane"
            ...
            @if ($selected !== null)
                @include('kopling-mail-client::messages.show', ['thread' => $selected])
            @else
