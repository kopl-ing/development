...
`k-core/src/Ux/` holds Kopling's re-usable, themeable UX components — the `<x-k::*>` Blade component library described in the charter — and the theming logic behind them (the daisyUI theme, CSS variables, brand tokens). `Ux/views/` (Blade templates) and the component PHP classes live together under `Ux/`. Compiled CSS/JS source (`k-core/resources/css/`, `k-core/resources/js/`) lives at the package root instead, same shape every `k-extensions/*` package uses for its own compiled assets — see "Source assets" below. There is no separate component-registry document yet: the charter (`public/charter.html`, Decision Log and Open Questions) is the design-system registry for now — what components exist, their contracts, and open decisions about them are tracked there until there's enough of them to justify a dedicated reference doc.
...
## Prefer htmx 4's own features over hand-rolled or outdated equivalents

Before reaching for custom JS or a pattern remembered from older htmx (a manual fetch, a bespoke event bus, a hand-rolled "swap two regions" hack), check whether htmx 4 already has a first-party way to do it — `hx-boost`, `hx-status:*`, `<hx-partial>`, `innerMorph`/`outerMorph`, `hx-preload`, `hx-optimistic`, etc. (`node_modules/htmx.org/dist/skills/htmx-guidance.md` has the full list). This project intentionally tracks htmx 4, not htmx 1/2 — solving something with a workaround that htmx 4 already solves natively defeats the point of being on it.
...
`k-core`'s own CSS/JS source lives at `k-core/resources/css/app.css` and `k-core/resources/js/app.js` — at the package root, not nested under `Ux/` or a monorepo-root `resources/`. Every `k-extensions/*` package that needs compiled assets uses the exact same shape: its own `resources/css/`, `resources/js/`, compiled into its own `dist/`. The root `package.json`/`vite.config.js`/`vite.core-dist.config.js` are a single shared Node toolchain for convenience (one `node_modules`, not one per package), but every config points *into* the owning package's own source tree and *into* that package's own `dist/` for output — the root itself owns no source files. Rationale and alternatives considered: `.docs/planning/decisions.md` ("Source assets live inside the owning package's own domain folder..." and its 2026-07-29 reversal moving assets to each package's root).

## How k-core ships compiled assets (Node/npm never runs on a Kopling host)

The root Vite build (`vite.config.js`, `public/build/*`, sourced from `k-core/resources/**`) is monorepo-dev-only, consumed via Laravel's `@vite()` in dev. It is **not** how a real Kopling site gets its CSS/JS: `vite.core-dist.config.js` (`npm run build:core-dist`) and `.github/workflows/release.yml` compile `k-core/resources/css/app.css` + `resources/js/app.js` (and the other four bundles: `editor`, `emoji-picker`, `tag-input`, `icon-picker`) with fixed, unhashed filenames into `k-core/dist/{name}.css`+`{name}.js` at release time (`workflow_dispatch`, not on every push), commit that to `main`, then tag the release — which triggers `subsplit.yml`'s `create: tags:` handler, carrying the compiled assets into `kopl.ing/core`. Node/Vite only ever run in GitHub Actions, at release time — never on a Kopling site's host. `head.blade.php` loads all five via `Manager::viteOrDist(base_path('k-core'), $name)` for each of `Manager::CORE_COMPILED_BUNDLES` — `@vite()` when the monorepo's own dev manifest covers it, the committed `dist/` output otherwise, so `kopling/core` installed standalone (no `public/build/manifest.json` at all) no longer throws `ViteManifestNotFoundException`. `Manager::extensionAssets()` registers `k-core/dist/{name}.*` into the same runtime asset route extensions' own compiled assets use, so the fallback URLs `viteOrDist()` generates actually resolve to something. Rationale and alternatives considered: `.docs/planning/decisions.md` ("k-core ships precompiled CSS/JS as committed release artifacts..." and "Extensions get their own compiled-asset pipeline, mirroring k-core's").

## How extensions ship compiled assets

An extension that needs real Tailwind/daisyUI generation beyond the safelisted subset, or a bundled npm dependency, gets its own `resources/css/{name}.css` + `resources/js/{name}.js` — same shape as `k-core`'s own, compiled into its own `k-extensions/{name}/dist/{name}.css`+`{name}.js` via `vite.extension-dist.config.js` (parameterized by the `KOPLING_EXTENSION` env var, one generic config rather than one per extension — discovers every `resources/js/*.js` file inside that extension, not one hardcoded entry) and `.scripts/build-extension-assets.mjs` (`npm run build:extensions-dist`, discovers every extension with at least one `resources/js/*.js` present and builds each in turn — directory-convention opt-in, most extensions have none and are skipped). Wired into `release.yml` right after `build:core-dist`; `subsplit.yml` needs no changes, since it already mirrors whatever's committed in each split's own directory at tag time. An extension's own `app.css` must use `@import "tailwindcss" source(none);` plus an explicit `@source` scoped to only its own `views/` — without `source(none)`, Tailwind's automatic content-scan walks the whole monorepo regardless of which package is being built, defeating the entire point of a self-contained, third-party-installable bundle. Register per-`PortalExtension` with `PortalExtension::compiledAssets(string $extensionRoot, ?string $name = null)` (portal-scoped, like the existing hand-written `css()`/`js()` — not loaded unconditionally the way `k-core`'s own assets are) — `$name` defaults to this attachment's own Portal id (sanitized, `::`/`/` → `-`), falling back to plain `app` when no Portal-specific file exists, so an extension attaching to two different Portals ships two different bundles without colliding on one `app.js`; pass `$name` explicitly to override the derived name (e.g. two Portals deliberately sharing one bundle). `Manager::viteOrDist(string $extensionRoot, string $name = 'app')` renders `@vite()` when the monorepo's own dev build covers it, falling back to the committed `dist/` output otherwise. `k-extensions/example` is the working reference implementation, including a second Portal attachment demonstrating the auto-derived-name case — illustrative only, like everything else it demonstrates. Rationale and alternatives considered: `.docs/planning/decisions.md` ("Extensions get their own compiled-asset pipeline, mirroring k-core's").
...
Default to no comment. Code ships to production and gets loaded (by tooling, by AI agents reading it, by every future contributor) whether or not anyone asked for the essay — a docblock is not a substitute for self-explanatory naming, and it isn't free just because it "might help later." Write one only when a reader at that exact line would otherwise get it wrong: a real browser/CSS quirk, an ordering dependency, a "don't touch X without also touching Y" warning. **One sentence.** If it takes more than one to justify, that justification belongs in the PR description or `decisions.md`, not the file — write the short version here and move the rest there, don't split the difference with a paragraph. Reusing an existing pattern (e.g. wiring up another `hx-boost`+`hx-target`/`hx-select` pair, another `data-*` JS hook) needs only a name-drop of the precedent ("same target as `page/pagination.blade.php`"), not a re-explanation of how or why that pattern works — that belongs once, in this file's own Gotchas section or `decisions.md`, not re-derived at every call site.
...
- This project vendors **htmx 4** (`node_modules/htmx.org`, currently a beta), not htmx 1/2 — attribute inheritance is explicit (`hx-target:inherited="..."` on a parent; a plain `hx-target` never cascades to children) and lifecycle events use colon form (`htmx:after:swap`, not `htmx:afterSwap`). Check `node_modules/htmx.org/dist/skills/htmx-guidance.md` (and sibling `htmx-migration.md`/`htmx-upgrade-from-htmx2.md`) before assuming htmx 1/2 behavior. Also scope `hx-boost`/`hx-target` to the narrowest element that needs it (e.g. one `<nav>`), not a wide wrapper — boosting a broader region also intercepts unrelated same-origin links inside it.
- `htmx:after:swap` fires on the *element that triggered the request*, not the swap target — for an `outerHTML` swap of an ancestor, that element is already detached by the time the event fires, so even a `window` listener outside the swapped subtree never sees it. Listen for `htmx:after:settle` instead (fires on the actual swapped-in element, which does bubble) when something outside the swap needs to react to it.
- htmx scans for `hx-*` attributes exactly once, at `htmx.process()` time — an attribute added afterward (e.g. Alpine's `x-bind:hx-post` writing a value once it's known) needs an explicit `htmx.process(el)` call before it does anything, since there's no MutationObserver watching for it.
- A boosted element (`hx-boost`) with no explicit `hx-target` swaps `document.body`'s innerHTML from the response's `<body>` — fetch auto-follows redirects, so both a success redirect and a validation-error redirect-back-with-errors work transparently with zero controller changes. Pair `hx-target`+`hx-select` on the same selector (see `page/pagination.blade.php` or `card/title.blade.php`) instead when only a shared content region, not the whole shell, should swap.
- `route($name, $id, $extra)` does **not** merge `$extra` into the query string — the third positional arg is `$absolute` (bool). Pass every parameter, including extras like `page`, inside one array: `route($name, ['moment' => $id, 'page' => 2])`.
- Don't hardcode a row count in a test to force a second pagination page — a model's own `$perPage` can change out from under it. Derive it instead: `(new Model())->getPerPage() + 1`.
