...
The root Vite build (`vite.config.js`, `public/build/*`, sourced from `k-core/resources/**`) is monorepo-dev-only, consumed via Laravel's `@vite()`. It is **not** how a real Kopling site gets its CSS/JS: `vite.core-dist.config.js` (`npm run build:core-dist`) and `.github/workflows/release.yml` compile `k-core/resources/css/app.css` + `resources/js/app.js` with fixed, unhashed filenames into `k-core/dist/app.css` + `app.js` at release time (`workflow_dispatch`, not on every push), commit that to `main`, then tag the release — which triggers `subsplit.yml`'s `create: tags:` handler, carrying the compiled assets into `kopl.ing/core`. Node/Vite only ever run in GitHub Actions, at release time — never on a Kopling site's host. Rationale and alternatives considered: `.docs/planning/decisions.md` ("k-core ships precompiled CSS/JS as committed release artifacts...").

**TODO, not yet built:** `head.blade.php` (`k-core/views/layouts/partials/head.blade.php`) still calls `@vite()` unconditionally for `k-core`'s own assets, which throws `ViteManifestNotFoundException` for anyone who installs `kopling/core` standalone outside this monorepo (no `public/build/manifest.json` exists there). `Manager::viteOrDist()` (see "How extensions ship compiled assets" below) already implements exactly this fallback, just for extensions' compiled assets — retrofitting `head.blade.php`'s own `@vite()` calls onto it would close this TODO too, but hasn't been done yet since `viteOrDist()` currently assumes a single `app.css`+`app.js` pair, and `k-core` has five (`app`, `editor`, `emoji-picker`, `tag-input`, `icon-picker`). Do this before `kopling/core` is meant to be installed anywhere but this monorepo.

## How extensions ship compiled assets

An extension that needs real Tailwind/daisyUI generation beyond the safelisted subset, or a bundled npm dependency, gets its own `resources/css/app.css` + `resources/js/app.js` — same shape as `k-core`'s own, compiled into its own `k-extensions/{name}/dist/app.css`+`app.js` via `vite.extension-dist.config.js` (parameterized by the `KOPLING_EXTENSION` env var, one generic config rather than one per extension) and `scripts/build-extension-assets.mjs` (`npm run build:extensions-dist`, discovers every extension with a `resources/js/app.js` present and builds each in turn — directory-convention opt-in, most extensions have none and are skipped). Wired into `release.yml` right after `build:core-dist`; `subsplit.yml` needs no changes, since it already mirrors whatever's committed in each split's own directory at tag time. An extension's own `app.css` must use `@import "tailwindcss" source(none);` plus an explicit `@source` scoped to only its own `views/` — without `source(none)`, Tailwind's automatic content-scan walks the whole monorepo regardless of which package is being built, defeating the entire point of a self-contained, third-party-installable bundle. Register with `PortalExtension::compiledAssets(string $extensionRoot)` (portal-scoped, like the existing hand-written `css()`/`js()` — not loaded unconditionally the way `k-core`'s own assets are); `Manager::viteOrDist()` renders `@vite()` when the monorepo's own dev build covers it, falling back to the committed `dist/` output otherwise. `k-extensions/example` is the working reference implementation — illustrative only, like everything else it demonstrates. Rationale and alternatives considered: `.docs/planning/decisions.md` ("Extensions get their own compiled-asset pipeline, mirroring k-core's").
