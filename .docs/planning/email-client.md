...
- **Threading**: messages are grouped into conversations via the `References` header (RFC 5322 §3.6.4, walked oldest-first to find the thread's root ancestor; falls back to `In-Reply-To`, then the message's own `Message-ID`, when a client skipped `References`). The message list shows one card per thread (its latest message, with a reply-count badge); opening one shows every message in that thread stacked as cards, oldest first — visually modeled on a Moment + its Replies.
- **Deliberately not real Moments/Replies.** `Moment` has no privacy concept at all in this codebase — no visibility/scope field — so making an email thread an actual `Moment` row would make private correspondence visible to the whole community, and reachable by every extension that generically hooks into any Moment (reactions, tags, pins, moderation flagging, and ActivityPub federation's own "deliver new Moments" listener could try to broadcast it to the fediverse). Same anti-pattern this repo already has a named incident and rule against — reusing a model because its CRUD/UI is convenient, not because the domain concept matches. mail-client keeps owning its own data (`mail_messages`, grouped by `thread_id`); only the *visual* language (daisyUI `card` styling) is shared, not the underlying model. No "promote a thread into a real community Moment" feature for now — deliberately deferred, not ruled out.
